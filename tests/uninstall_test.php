...
/**
 * Tests for the local_playergames pre-uninstallation hook.
 *
...
/**
 * Tests for xmldb_local_playergames_uninstall(). Every table in db/install.xml is
 ...
 *
 * @covers ::xmldb_local_playergames_uninstall
 */
final class uninstall_test extends \advanced_testcase {
    ...
    protected function setUp(): void {
        ...
        parent::setUp();
        $this->resetAfterTest();
        require_once($CFG->dirroot . '/local/playergames/db/uninstall.php');
    }

    /**
     * Tests that the uninstall hook deletes only local_playergames-prefixed
    ...
    public function test_uninstall_deletes_only_own_prefixed_preferences(): void {
        ...
        set_user_preference('local_playergames_gamification', 1, $user);
        set_user_preference('local_aihub_openai_model', 'gpt', $user);
        set_user_preference('unrelated_pref', 'keep', $user);

        $result = xmldb_local_playergames_uninstall();

        $this->assertTrue($result);
        $this->assertFalse($DB->record_exists('user_preferences', [
            'userid' => $user->id,
            'name' => 'local_playergames_gamification',
        ]));
        $this->assertTrue($DB->record_exists('user_preferences', [
            'userid' => $user->id,
            'name' => 'local_aihub_openai_model',
        ]));
        $this->assertTrue($DB->record_exists('user_preferences', [
            'userid' => $user->id,
            'name' => 'unrelated_pref',
    ...
    public function test_uninstall_with_no_matching_preferences_is_a_noop(): void {
        $this->assertTrue(xmldb_local_playergames_uninstall());
    }
}
